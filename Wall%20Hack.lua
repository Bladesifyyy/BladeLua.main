--[[
	...
	Wall Hack Module [BladeLua] by Bladesify © CC0 1.0 Universal (2023)
...
if not getgenv().BladeLua or getgenv().BladeLua.WallHack then return end
...
local Environment = getgenv().BladeLua.WallHack
...
function Environment.Functions:Exit()
	...
	getgenv().BladeLua.WallHack.Functions = nil
	getgenv().BladeLua.WallHack = nil
...
function Environment.Functions:ResetSettings()
	Environment.Visuals = {
		...
		BoxSettings = {
			Enabled = true,
			Type = 1; -- 1 - 3D; 2 - 2D
			Color = Color3fromRGB(255, 87, 233),
			Transparency = 0.7,
			Thickness = 1,
			Filled = false, -- For 2D
			Increase = 1
		...
		HeadDotSettings = {
			Enabled = true,
			Color = Color3fromRGB(222, 87, 255),
			Transparency = 0.5,
			Thickness = 1,
			Filled = true,
			Sides = 30
		...
		HealthBarSettings = {
			...
			Size = 2,
			Offset = 10,
			OutlineColor = Color3fromRGB(34, 255, 0),
			Blue = 50,
			Type = 3 -- 1 - Top; 2 - Bottom; 3 - Left; 4 - Right
		}
	}
